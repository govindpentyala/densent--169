...
def get_study_level_data(study_type):
    ...
    for phase in data_cat:
        BASE_DIR = rf'C:\Users\kumar govind\Downloads\Ml\nlp\MURA-v1.1\train\{study_type}'

        # Print BASE_DIR for debugging
        print(f"BASE_DIR: {BASE_DIR}")

        # Use os.walk to get the list of subdirectories
        walk_result = list(os.walk(BASE_DIR))

        # Check if the walk_result list is not empty
        if len(walk_result) == 0:
            print(f"No patient folders found in {BASE_DIR}.")
            continue  # Skip this phase if there are no patients

        # Extract patient folder names
        patients = walk_result[0][1]
        ...
        for patient in tqdm(patients):  # for each patient folder
            for study in os.listdir(BASE_DIR + '/' + patient):  # for each study in that patient folder
                ...
                path = BASE_DIR + '/' + patient + '/' + study + '/'  # path to this study # path to this study
